...
// extension. The `ts` extension means this JavaScript is a TypeScript file, which, among other things,
...
import { handleErrorsMiddleware } from './utils'
import { RateLimiter, RateLimiterClient, ChatRoom } from './dos'
...
app.post('/api/room', (c) => {
  ...
  console.log('POST api/room', id.toString())
  return new Response(id.toString(), {
    headers: { 'Access-Control-Allow-Origin': '*' },
  })
})

app.get('/api/room/:name/websocket', async (c) => {
  const name = c.req.param('name')
  console.log('get api/room/:name/websocket', name)
  // This is a request for `/api/room/<name>`. The name is passed in the URL as a parameter.
  ...
  console.log('new url port', newUrl.port)
  // const _curPath = newUrl.pathname.split('/')
  newUrl.pathname = '/websocket'

  // Send the request to the object. The `fetch()` method of a Durable Object stub has the
  // same signature as the global `fetch()` function, but the request is always sent to the
  // object, regardless of the request's URL.
  console.log('fetching room object', newUrl.toString())
  return roomObject.fetch(newUrl, c.req.raw)
})
